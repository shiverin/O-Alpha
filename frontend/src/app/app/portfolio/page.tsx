...
export default function PortfolioPage() {
  ...
  const { data: serverPositions } = useSWR<ServerPositionMetrics[]>(
    currentUserID !== 999 ? `http://localhost:8080/api/v1/user/portfolio/positions?user_id=${currentUserID}` : null,
    fetcher
  );

  // 🔀 DUAL-MODE DISPATCH MATRIX (AUTO-FALLBACK FOR DEMO GUEST 999)
  const totalAssetValue = currentUserID === 999 || !serverSummary ? mockSummary.totalAssetValue : serverSummary.total_asset_value;
  const changePercent24h = currentUserID === 999 || !serverSummary ? mockSummary.changePercent24h : serverSummary.change_percent_24h;
      ...
      <div className="w-full max-w-full min-w-0 bg-transparent flex flex-col gap-6 md:gap-10 animate-in fade-in duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] overflow-hidden">
        ...
        <section className="w-full min-w-0 max-w-full">
          <div className="group relative bg-surface-container-low border border-outline-variant/30 rounded-[32px] p-5 sm:p-8 xl:p-10 overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.3)]">
            ...
            <div className="absolute inset-0 opacity-[0.12] pointer-events-none transition-transform duration-1000 group-hover:scale-105" style={{ backgroundImage: "radial-gradient(circle at 2px 2px, rgba(255,255,255,0.15) 1px, transparent 0)", backgroundSize: "32px 32px" }} />
              ...
              <div className="w-full xl:w-auto min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2.5 mt-3 sm:mt-4">
                  <span className="bg-primary-fixed-dim/10 text-primary-fixed-dim font-mono text-[11px] px-2.5 py-0.5 rounded-full flex items-center gap-1 border border-primary-fixed-dim/20 shadow-[0_0_12px_rgba(0,240,255,0.15)]">
                    ...
                    {changePercent24h >= 0 ? "+" : ""}{changePercent24h}% (24h)
                  </span>
                  <span className="text-on-surface-variant/50 font-mono text-[11px] whitespace-nowrap">
                    {changeDollar24h >= 0 ? "+" : ""}${changeDollar24h.toLocaleString()}
                  </span>
                </div>
              </div>
                ...
                <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 400 100">
                  <defs>
                    <linearGradient id="cyan-fade" x1="0" x2="0" y1="0" y2="1">
                      <stop offset="0%" stopColor="rgba(0, 219, 233, 0.15)"></stop>
                      <stop offset="100%" stopColor="rgba(0, 219, 233, 0)"></stop>
                    </linearGradient>
                  </defs>
                  <path d={`${sparklinePath} L 400 100 L 0 100 Z`} fill="url(#cyan-fade)"></path>
                  <path d={sparklinePath} fill="none" stroke="#00dbe9" strokeWidth="1.5" style={{ filter: "drop-shadow(0 0 6px rgba(0,219,233,0.4))" }}></path>
                  <circle cx="400" cy="20" fill="#00dbe9" r="3.5" style={{ filter: "drop-shadow(0 0 6px #00dbe9)" }}></circle>
                </svg>
        ...
        <section className="w-full min-w-0 max-w-full grid grid-cols-1 xl:grid-cols-12 gap-6 md:gap-8 items-start">
          <div className="w-full min-w-0 md:col-span-12 xl:col-span-5 bg-surface-container-low border border-outline-variant/30 rounded-[32px] p-5 sm:p-8 flex flex-col justify-between h-auto min-h-[380px] sm:min-h-[420px] xl:min-h-[440px]">
            <h3 className="text-xs font-light tracking-wide text-on-surface border-b border-outline-variant/20 pb-4 mb-2">Composition</h3>
              ...
              {compositionSegmentsList.map((segment, idx) => (
                <div key={idx} className="flex justify-between items-center group cursor-default">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <span className="text-on-surface-variant/80 group-hover:text-on-surface transition-colors truncate">{segment.label}</span>
                  </div>
                  <span className="text-on-surface/40 group-hover:text-on-surface transition-colors shrink-0">({segment.percentage}%)</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="w-full min-w-0 max-w-full md:col-span-12 xl:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8 items-start">
            <div className="w-full min-w-0 bg-surface-container-low border border-outline-variant/30 rounded-[32px] p-5 sm:p-6 min-h-[180px] sm:min-h-[200px] flex flex-col justify-between hover:bg-surface-container transition-colors duration-300">
              <div className="flex justify-between items-start mb-4">
                <div className="text-primary-fixed-dim"><Icon name="trending_up" /></div>
                <span className="bg-white/[0.02] border border-outline-variant/20 px-2.5 py-0.5 rounded-full font-mono text-[9px] tracking-wider text-on-surface-variant/60 uppercase shrink-0">Alpha Gen</span>
              </div>
              <div>
            ...
            <div className="w-full min-w-0 bg-surface-container-low border border-outline-variant/30 rounded-[32px] p-5 sm:p-6 min-h-[180px] sm:min-h-[200px] flex flex-col justify-between hover:bg-surface-container transition-colors duration-300 relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="text-secondary-fixed-dim"><Icon name="shield" /></div>
                <span className="bg-white/[0.02] border border-outline-variant/20 px-2.5 py-0.5 rounded-full font-mono text-[9px] tracking-wider text-on-surface-variant/60 uppercase shrink-0">Metrics</span>
              </div>
              <div>
            ...
            <div className="w-full min-w-0 sm:col-span-2 bg-surface-container-low border border-outline-variant/30 rounded-[32px] p-5 sm:p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-surface-container transition-colors duration-300">
              <div className="min-w-0">
                <p className="font-mono text-[10px] tracking-wider text-on-surface-variant/50 uppercase mb-1">Estimated Annual Yield</p>
                <p className="text-2xl sm:text-3xl font-light text-on-surface truncate">
                  ${estimatedYield.toLocaleString()}<span className="text-on-surface-variant/30 text-lg">.00</span>
                </p>
              </div>
              <div className="w-full sm:w-auto text-left sm:text-right shrink-0">
                <p className="font-mono text-[10px] text-primary-fixed-dim tracking-wider uppercase mb-1.5 sm:mb-2">Target Met</p>
                <div className="w-full sm:w-36 h-[3px] bg-surface-container-highest rounded-full overflow-hidden">
                  <div className="h-full bg-primary-fixed-dim shadow-[0_0_10px_rgba(0,219,233,0.8)] rounded-full" style={{ width: `${progressPercent}%` }} />
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="w-full min-w-0 max-w-full bg-surface-container-low border border-outline-variant/30 rounded-[32px] pb-4">
          <div className="p-5 sm:p-6 border-b border-outline-variant/20 flex justify-between items-center bg-white/[0.01]">
            <h3 className="text-sm sm:text-md font-light tracking-wide text-on-surface">Positions</h3>
            ...
            <table className="w-full text-left border-collapse min-w-[700px] table-fixed">
              <thead>
                <tr className="font-mono text-[10px] tracking-wider uppercase text-on-surface-variant/40 border-b border-outline-variant/20 bg-void-black/20">
                  ...
                  <th className="p-4 sm:p-6 font-medium w-[18%]">Current Price</th>
                  <th className="p-4 sm:p-6 font-medium text-right w-[20%]">Unrealized P&L</th>
                  <th className="p-4 sm:p-6 font-medium text-right w-[17%]">Exposure</th>
                </tr>
              </thead>
                      ...
                      <tr key={idx} className="hover:bg-surface-container-high/30 transition-colors duration-200 group">
                        <td className="p-4 sm:p-6 min-w-0">
                          <div className="flex items-center gap-3.5 min-w-0">
                            <div className={`w-8 h-8 rounded-full bg-void-black border border-outline-variant/20 flex items-center justify-center font-mono text-[10px] text-on-surface/60 shrink-0 ${position.borderClass || ""}`}>{position.initials}</div>
                            <div className="min-w-0 truncate">
                              <p className="font-medium group-hover:text-primary-fixed-dim transition-colors duration-300 truncate">{position.name}</p>
                              <p className="font-mono text-[10px] text-on-surface-variant/40 mt-0.5 truncate">{position.symbol} • {position.category}</p>
                            </div>
                          </div>
                        </td>
                        <td className="p-4 sm:p-6 font-mono font-light text-on-surface-variant truncate">{position.allocation}%</td>
                        <td className="p-4 sm:p-6 font-mono font-light text-on-surface-variant truncate">${position.currentPrice.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                        <td className="p-4 sm:p-6 text-right shrink-0">
                          <span className={`font-mono font-medium truncate ${position.isPositive ? "text-primary-fixed-dim drop-shadow-[0_0_6px_rgba(0,219,233,0.25)]" : "text-error"}`}>
                            {position.isPositive ? "+" : ""}${position.unrealizedPnL.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          </span>
                        </td>
                        ...
                        <tr key={idx} className="hover:bg-surface-container-high/30 transition-colors duration-200 group">
                          ...
                          <td className="p-4 sm:p-6 text-right shrink-0">
                            <span className={`font-mono font-medium truncate ${isPositive ? "text-primary-fixed-dim drop-shadow-[0_0_6px_rgba(0,219,233,0.25)]" : "text-error"}`}>
                              {isPositive ? "+" : ""}${position.unrealized_pnl.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            </span>
                          </td>
                          <td className="p-4 sm:p-6 text-right font-mono font-light tracking-tight text-on-surface-variant truncate">${position.exposure.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                        </tr>
