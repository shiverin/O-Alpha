    ...
    <AppShell title="Activity Console">
      <div className="w-full bg-transparent flex flex-col gap-6 md:gap-10 animate-in fade-in duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 pb-2">
          <div>
            <h1 className="text-2xl sm:text-3xl font-light tracking-tight text-on-surface">Execution Stream</h1>
            <p className="text-xs sm:text-sm font-light text-on-surface-variant/70 max-w-2xl mt-1">
              Real-time audit log of systematic trading actions, strategy recalibrations, and critical system alerts.
            </p>
          </div>
                  ...
                  <tbody className="font-mono text-[11px] tracking-wide text-on-surface/90 divide-y divide-outline-variant/10">
                    {mockExecutionStream.map((log, index) => (
                      <tr key={index} className="transition-colors duration-150 hover:bg-white/[0.01] cursor-default">
                        <td className="py-4 px-6 text-on-surface-variant/60">{log.timestamp}</td>
                        <td className="py-4 px-6">
                          <span className={log.actionColorClass}>{log.action}</span>
                        </td>
                        <td className="py-4 px-6 font-medium text-on-surface">{log.asset}</td>
                        <td className="py-4 px-6 text-right text-on-surface-variant">{log.price}</td>
                        <td className="py-4 px-6 text-right text-on-surface-variant">{log.size}</td>
                        <td className={`py-4 px-6 text-right ${log.action.startsWith("BUY") ? "text-primary-fixed-dim" : "text-on-surface-variant/40"}`}>
                          {log.slippage}
                        </td>
                        <td className="py-4 px-6">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-sm border font-medium text-[9px] tracking-wider ${log.statusColorClass}`}>
            ...
            <div className="rounded-[24px] bg-surface-container-low border border-outline-variant/30 p-6 relative overflow-hidden group">
              ...
              <div className="flex flex-col gap-4">
                {mockSystemAlerts.map((alert) => (
                  <div key={alert.id} className={`flex gap-4 p-4 rounded-xl bg-void-black/20 border border-outline-variant/10 border-l-2 ${alert.borderClass}`}>
