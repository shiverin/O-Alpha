...
func main() {
	...
	// Update schema_migrations to reflect actual migrations
	if version == 0 {
		fmt.Println("\nUpdating migration versions to match applied migrations...")
		...
		fmt.Println("✅ Updated migration versions to 1 and 2")
		
		// Verify update
		rows, err = db.Query(context.Background(), "SELECT version, dirty FROM schema_migrations ORDER BY version")
		if err != nil {
			log.Fatalf("Failed to list migrations after update: %v", err)
		}
		defer rows.Close()
		
		fmt.Println("Applied migrations after update:")
		for rows.Next() {
			var v uint64
			var d bool
			if err := rows.Scan(&v, &d); err != nil {
